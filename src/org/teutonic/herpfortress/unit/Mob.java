...
import org.teutonic.herpfortress.*;
import org.teutonic.herpfortress.ai.PathFinder;
import org.teutonic.herpfortress.entity.*;
import org.teutonic.herpfortress.particle.*;
import org.teutonic.herpfortress.unit.order.*;
import org.teutonic.herpfortress.weapon.*;
...
public class Mob extends Unit {
	...
	public static Mob create(int unitClass, Player player) {
		...
		if (unitClass == 0) unit = new Scout(player);
		if (unitClass == 1) unit = new Soldier(player);
		if (unitClass == 2) unit = new Pyro(player);
		if (unitClass == 3) unit = new Demoman(player);
		if (unitClass == 4) unit = new Heavy(player);
		if (unitClass == 5) unit = new Engineer(player);
		if (unitClass == 6) unit = new Medic(player);
		if (unitClass == 7) unit = new Sniper(player);
		if (unitClass == 8) unit = new Spy(player);
